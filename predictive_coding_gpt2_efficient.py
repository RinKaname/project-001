...
def get_gpt2_small_config():
    """
    GPT-2 Small parameter equivalent (125M parameters).
    Because we use weight tying and a simpler PC layer, we need 1536 hidden size
    and 20 layers to hit exactly 124.6 Million parameters.
    """
    return ModelConfig(
        hidden_size=1536,
        num_layers=20,
...
class PCLayer:
    ...
    def __init__(self, config: ModelConfig, layer_idx: int):
        ...
        # Memory optimization: Initialize directly in bfloat16
        self.W = torch.empty(self.hidden_size, self.hidden_size, dtype=torch.bfloat16).uniform_(-bound, bound)
        self.b = torch.zeros(self.hidden_size, dtype=torch.bfloat16)
...
class PCLanguageModel:
    ...
    Utilizes Weight Tying and bfloat16 for extreme memory efficiency.
    """
    def __init__(self, config: ModelConfig):
        self.config = config

        # Embeddings (in bfloat16)
        self.token_emb = torch.empty(config.vocab_size, config.hidden_size, dtype=torch.bfloat16)
        torch.nn.init.normal_(self.token_emb, std=0.02)

        # PC Layers
        self.layers = [PCLayer(config, i) for i in range(config.num_layers)]

        # Weight Tying: We DO NOT create an lm_head matrix.
        # We will re-use token_emb.T for the final readout.
    ...
    def print_parameter_summary(self):
        ...
        # LM Head (Weight Tied)
        # We don't add to parameter count, but print a note
        print(f"{'LM Head':<25} | {'Weight Tied':<15} | {'[Tied to Emb]':<20} | {'0':<12} | {'0.00 KB':<10} | N/A")
    ...
    def train_step(self, input_ids):
        ...
        # 4. Final Output Prediction (Weight Tied LM Head Update)
        # Shift targets for next-token prediction
        targets = input_ids[:, 1:].contiguous()
        top_state = states[-1][:, :-1, :].contiguous() # Drop last token state

        # Weight Tying: Use token_emb instead of lm_head
        logits = torch.matmul(top_state.float(), self.token_emb.float().T)

        # Custom backward for tied head (since we can't use autograd)
        ...
        # Because of weight tying, the gradient at the top must update self.token_emb.
        # d_tied_head = d_logits^T @ top_state
        d_logits_flat = d_logits.view(-1, self.config.vocab_size).to(torch.bfloat16)
        top_state_flat = top_state.view(-1, self.config.hidden_size).to(torch.bfloat16)

        delta_tied_head = torch.matmul(d_logits_flat.T, top_state_flat)
        self.token_emb.sub_(delta_tied_head, alpha=self.config.eta_w * 10)

        # Sensory embedding update (bottom layer prediction error)
        ...
        # Cross entropy needs float32 for stability
    ...
    def generate(self, input_ids, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            # Compute logits using tied weights (cast to float32 for stability during sampling)
            logits = torch.matmul(top_state_last.float(), self.token_emb.float().T)
